...
from choiloader_bert import ChoiDataset, collate_fn
...
from wiki_loader_bert import WikipediaDataSet
import accuracy
import numpy as np
from termcolor import colored

# import custom output bert model wrapper
from custom_output_bert_model import FeatureExtractorBert
...
def import_model(model_name, embedding_size):
    ...
    module.INPUT_SIZE = embedding_size
...
def main(args):
    ...
    if not args.infer:
        if args.wiki:
            ...
            dev_dataset = ChoiDataset(dataset_path, manifesto=True, bert_model=bert_model)
            test_dataset = ChoiDataset(dataset_path, manifesto=True, bert_model=bert_model)
        
        else:
            dataset_path = utils.config['choidataset']
            train_dataset = ChoiDataset(dataset_path, bert_model=bert_model)
            dev_dataset = ChoiDataset(dataset_path, bert_model=bert_model)
            test_dataset = ChoiDataset(dataset_path, bert_model=bert_model)
    ...
    if args.model:
        model = import_model(args.model, args.embedding_size)
    elif args.load_from:
